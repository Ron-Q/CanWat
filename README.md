...
CanWat can be downloaded from the GitLab repository as a packed file. Go to <https://gitlab.mn.tu-dresden.de/rqueck/canwat>, on the top left side you will find a button with a download symbol. The package contains the preferable folder structure and the source code. Please unpack the files in any folder and remember it, as it is the CanWat home folder. 
