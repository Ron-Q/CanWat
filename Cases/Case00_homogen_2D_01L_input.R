#+
# NAME*:        Case00_homogen_2D_01L_input.R
# PURPOSE*:     CanWat input parameters test case
# RELEVANCY*:   CanWat
# INPUTS*:      most of the code is input
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  # CWpara_name <- paste0("CW_parameter-",treetype, "SF.R")
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  #* name and range ####
  fnam.SD <- "homogen_2D_01L.csv" # -, name of the static driver
  SD.dims = c( 1, 10,  10)      # -, [z,y,x]  dimensions
  dz <- 10; dy <- 1; dx <- 1    # m, grid resolution     
  ...
  x1.ref <-  0;  x2.ref <-  10  # east of the reference point
  y1.ref <-  0;  y2.ref <-  10  # north of the reference point
  z1.ref <-  0;  z2.ref <-  10     
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  fnam.MetData <- "Case03_DynD.csv" # -, name of the dynamic driver
  # fnam.MetData <- "Case01_DynD.csv" # -, contains a bigger event
  # fnam.MetData <- "Case01_DynD2.csv" # -, contains 2 subsequent events
  dt_met <- 600                     # s, length of a time step
  
  #* Event Selection ####
  it1 <-   1;  it2 <-   288          # number of time steps, selected interval
...
#* User extension of the Standard output ####
   ...
   output.mean.usr <- c("rH_3D", "rLE_3D", "rw_3D", "rET_3D", "rc_3D", "rb_3D", "rab")
...
# Numerical Output #########
  ...
  xcut.out <- NA  # NA = no output or a x grit number = yz-cross-section at xcut.out
  ycut.out <- NA  # NA = no output or a y grit number = xz-cross-section at ycut.out
  zcut.out <- NA  # NA = no output or a z grit number = xy-cross-section at zcut.out
