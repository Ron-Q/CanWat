...
gca <- 809 ; gcb <- 20.4;  gcc <- 15.8; gcd <- 0.318  #; gcmax <- 0.0246 # Flussneu.mdb ASTW
if (!exists("gcmax")) gcmax <- 0.0246                                                 # cat("rcmin =", 1/gcmax)

#* Storage ####
#* parameter that regulate the resistance network, ie. divide the available energy between ET, EV and H 
sc.min <- 0.0  # set to zero, as it is obsolete
               # part of storage from that evaporation occurs at the least, if there is water in the storage
...
sc.f <- 2      # non-linearity of the drying process, i.e. of the saturation of the air near the surface, check with the following line
               # sc.f <- 2; sc0 <- (0:100)/100;  plot(sc0, (1-exp(-sc0*sc.f))/(1-exp(-sc.f))); abline(a=0, b=1, h=c(0,1), v=c(0,1))
