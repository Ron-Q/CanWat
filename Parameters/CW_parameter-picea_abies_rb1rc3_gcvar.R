...
if (!exists("gcmax")) gcmax <- 0.0027                                                 # cat("rcmin =", 1/gcmax)
...
if (!exists("sc.min")) sc.min <- 0.02 # part of storage from that evaporation occurs at the least, if there is water in the storage
               # be careful, increasing sc.min reduces the resistance and increases ETR
if (!exists("sc.f")) sc.f <- 10      # non-linearity of the drying process, i.e. of the saturation of the air near the surface, check with the following line
